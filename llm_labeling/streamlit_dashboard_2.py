...
with st.sidebar:
    ...
    uploaded_files = st.file_uploader(
        "Upload one or more CSV files with DOI and abstract columns",
        type="csv",
        accept_multiple_files=True,
        help="Each CSV file must contain 'doi' and 'abstract' columns"
    )
            ...
            for uploaded_file in uploaded_files:
                try:
                    ...
                    if 'doi' not in df.columns or 'abstract' not in df.columns:
                        st.error(f"Required columns missing in '{uploaded_file.name}'. Available columns: " + ", ".join(df.columns))
                        continue
                    
                    # Count valid entries (non-null DOIs and abstracts)
                    valid_entries = df[df['doi'].notna() & df['abstract'].notna()].shape[0]
                    if valid_entries == 0:
                        st.error(f"No valid entries found in '{uploaded_file.name}'")
                        continue
                    ...
                    st.error(f"The file '{uploaded_file.name}' appears to be empty.")
                except Exception as e:
                    st.error(f"Error reading the file '{uploaded_file.name}': {str(e)}")
            
            # Display combined statistics
            if not combined_df.empty:
                st.success(f"✅ Found {all_valid_entries:,} valid papers across {len(uploaded_files)} files")
                
                # Display file-specific statistics in an expander
                ...
                max_allowed = min(all_valid_entries, MAX_PAPERS)
                ...
                if selection_method == "Process All":
                    sample_size = min(all_valid_entries, MAX_PAPERS)
                ...
                # Get valid papers dataframe
                valid_df = combined_df[combined_df['doi'].notna() & combined_df['abstract'].notna()]
                
                # Sample papers based on selection method
                ...
                **Processing Details**:
                ...
                - Files being processed: {len(uploaded_files)}
                """)
            else:
...
if st.button("Start Analysis", type="primary") and uploaded_files:
    ...
    try:
        ...
        if all_results:
            try:
                ...
                # Safely split the response string, handling potential errors
                try:
                    results_df[task_cols] = results_df['gemini_response'].str.split(';', expand=True)
                except Exception as e:
                    st.warning(f"Error splitting responses into columns: {str(e)}")
                    # Create empty columns if split fails
                ...
                # Map detailed labels with error handling
                try:
                    ...
                    st.warning(f"Error mapping detailed labels: {str(e)}")
                
                try:
                    # Drop intermediate task columns
                    results_df.drop(columns=[col for col in task_cols if col in results_df.columns], inplace=True)
                except Exception as e:
                    st.warning(f"Error dropping intermediate columns: {str(e)}")
                ...
                try:
                    ...
                    # Merge the results with the original dataframe, keeping all papers
                    final_df = original_df.merge(results_df, on='doi', how='left')
                    
                    # Fill NaN values with error message
                    for col in final_df.columns:
                        if col != 'doi':
                            final_df[col] = final_df[col].fillna("ERROR: Processing failed")
                    
                    # Reorder columns to put analysis results after original columns
                    analysis_cols = [
                        "source_file",  # Add source file to analysis columns
                        "gemini_response",
                    ...
                    # Create final column order, only including columns that exist
                    final_cols = ['doi'] + original_cols + [col for col in analysis_cols if col in final_df.columns]
                    
                    # Reorder columns
                    final_df = final_df[final_cols]
                except Exception as e:
                    st.error(f"Error merging results with original data: {str(e)}")
                    # If merge fails, use just the results dataframe
                    final_df = results_df
                
                # Display results
                with results_container:
                    st.header("Analysis Results")
                    total_papers = len(sampled_df)
                    successful_papers = sum(1 for r in all_results if "ERROR" not in str(r.get('gemini_response', '')))
                    st.write(f"Processed {total_papers} papers, {successful_papers} successful analyses")
                    ...
                    try:
                        ...
                        # Also provide option to download raw results
                        raw_results_df = pd.DataFrame(all_results)
                        raw_csv = raw_results_df.to_csv(index=False)
                        st.download_button(
                            label="📥 Download Raw Results CSV",
                            data=raw_csv,
                            file_name=f"gemini_raw_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                            mime="text/csv"
                        )
                    except Exception as e:
                        st.error(f"Error creating download buttons: {str(e)}")
                        # Provide basic text download as fallback
                        st.text_area("Raw Results (Copy and paste to save)", str(all_results))
                    
                    # Update the rate limit usage display
                    st.info(f"""
                    Processing Statistics:
                    - Total papers attempted: {total_papers:,}
                    - Successfully processed: {successful_papers:,}
                    - Success rate: {(successful_papers/total_papers*100):.1f}%
                    - API calls used: {num_batches:,} out of {REQUESTS_PER_DAY:,} daily limit
                    - Average papers per request: {len(all_results) / num_batches:.1f}
                    ...
                    with metric_cols[0]:
                        st.metric("Papers Processed", f"{total_papers:,}")
                    with metric_cols[1]:
                        st.metric("Processing Time", f"{(time.time() - start_time)/60:.1f} min")
                    with metric_cols[2]:
                        st.metric("API Calls Used", f"{num_batches:,}")
                    with metric_cols[3]:
                        success_rate = successful_papers / total_papers * 100
                        st.metric("Success Rate", f"{success_rate:.1f}%")
            
            except Exception as e:
                st.error(f"Error processing results: {str(e)}")
                # Fallback: Display raw results
                st.warning("Displaying raw results due to processing error")
                raw_df = pd.DataFrame(all_results)
                st.dataframe(raw_df)
                
                # Provide raw results download
                try:
...
if uploaded_files:
    try:
        # Initialize variables for token estimation
        total_tokens = 0
        token_counts = []
        sample_size = 5
        samples_processed = 0
        ...
        for uploaded_file in uploaded_files:
            try:
                ...
                if not first_char:
                    st.sidebar.error(f"The file '{uploaded_file.name}' is empty.")
                    continue
                ...
                for encoding in ['utf-8', 'latin1', 'iso-8859-1']:
                    try:
                        ...
                        st.sidebar.error(f"The file '{uploaded_file.name}' appears to be empty or corrupted.")
                        break
                    except Exception as e:
                        continue
                ...
                if 'df' not in locals():
                    st.sidebar.error(f"Could not read '{uploaded_file.name}' with any supported encoding.")
                    continue
                elif df.empty:
                    st.sidebar.error(f"The file '{uploaded_file.name}' contains no data.")
                    continue
                elif 'abstract' not in df.columns:
                    st.sidebar.error(f"Required 'abstract' column not found in '{uploaded_file.name}'.")
                ...
                if not valid_abstracts.any():
                    st.sidebar.error(f"No valid abstracts found in '{uploaded_file.name}'.")
                    continue
                
                # Calculate token estimates for a sample of abstracts
                valid_df = df[valid_abstracts].copy()
                file_sample_size = min(sample_size - samples_processed, len(valid_df))
                if file_sample_size > 0:
                    sample_df = valid_df.sample(n=file_sample_size)
                    ...
                    for abstract in sample_df['abstract']:
                        try:
                            ...
                            st.sidebar.warning(f"Error counting tokens in '{uploaded_file.name}': {str(e)}")
        ...
        # Display token statistics if we have any samples
        if token_counts:
            ...
            st.sidebar.info(f"""
            Token Estimates (based on {len(token_counts)} sample abstracts):
            ...
            st.sidebar.warning("Could not estimate tokens - no valid abstracts found.")
            
    except Exception as e:
        st.sidebar.error(f"Error estimating tokens: {str(e)}")
        st.sidebar.info("Please ensure you've uploaded valid CSV files.") 