...
from featureEngineering import country
...
class ETLPipeline():
    def __init__(self, api_key, credentials_json, project_id, limit=None, batch_size=100):
            ...
            self.study_types = [item for subcat in self.labels['study_types'].values() for method_list in subcat.values() for item in method_list]
            self.poverty_contexts = list(self.labels['poverty_contexts'].values())
            self.mechanisms = [item for subcat in self.labels['mechanisms'].values() for item in subcat]
            self.behaviors = [item for subcat in self.labels['Behaviors'].values() for item in subcat]
    ...
    def run(self, start_date, end_date):
        ...
        try:
            ...
            with st.spinner("Extracting Countries..."):   
            # Check for empty papers
                ...
                papers_df['country_of_study'] = papers_df['abstract'].apply(
                    self._country_extractor(method='spacy')
                )
